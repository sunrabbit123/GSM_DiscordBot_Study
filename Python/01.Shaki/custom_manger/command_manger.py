...
class command_manger:
    def __init__(self, message, prefixed=True):
        ...
        try:
            getattr(self, "command_custom" + self.features[message.content.split()[2]])(
    ...
    async def command_custom_delete(self, message, prefixed=True):
        ...
        try:
            searching = self.dbmanger.search_data(
                "made_command", "keycommand", contents
            )
        except (IndexError):
            pass  # 추후에 커맨드 도움말 추가
        else:
            ...
            command_server = [
                command for command in searching if command.server == server
            ]
            if command_server:
